            ...
            @if ($filters['type'] == 'category')
                <div class="my-3 md:my-5 text-lg md:text-xl lg:text-2xl xl:text-3xl">
                    @foreach ($categories as $category)
                        ...
                        @else
                            <a href="{{ route('programacao-adulto') }}" class="inline-block px-0.5 lg:px-1 border-4 {!! $filters['value'] == $category->name ? ' border-azul' : 'border-white' !!}"><img class="h-[50px] sm:h-[70px] md:h-[100px] lg:h-[120px]" src="{{ asset('images/icons/categories/' . $category->slug . '.svg') }}" alt="{{ $category->name }}"></a>
                        @endif
                    @endforeach
                </div>
            @endif
