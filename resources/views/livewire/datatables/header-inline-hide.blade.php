...
<div class="@if($column['hidden']) hidden @else relative h-12 overflow-hidden align-top flex table-cell @endif" @include('datatables::style-width')>
    ...
    @if($column['sortable'])
        <button wire:click="sort('{{ $index }}')"
                class="w-full h-full px-6 py-3 border-b border-gray-200 bg-gray-50 text-xs leading-4 font-bold text-gray-700 uppercase tracking-wider flex justify-between items-center focus:outline-none">
            ...
            @if($sort === $index)
                @if($direction)
                    <x-icons.chevron-up class="h-6 w-6 text-emerald-500 stroke-current" />
                @else
                    <x-icons.chevron-down class="h-6 w-6 text-emerald-500 stroke-current" />
                @endif
            @endif
            </span>
        </button>
    @else
        <div class="w-full h-full px-6 py-3 border-b border-gray-200 bg-gray-50 text-xs leading-4 font-bold text-gray-700 uppercase tracking-wider flex justify-between items-center focus:outline-none">
            <span class="inline flex-grow @if($column['headerAlign'] === 'right') text-right @elseif($column['headerAlign'] === 'center') text-center @endif"">{{ str_replace('_', ' ', $column['label']) }}</span>
        </div>
    @endif
